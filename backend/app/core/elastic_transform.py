...
def build_results_dataframe(raw_es_results, columns=[]):
    ...
    data_frame['major_version'] = data_frame['upstream_job'].apply(
        _get_version_from_job)
    data_frame['build_version'] = data_frame['cluster_version']
    data_frame['ocp_profile'] = data_frame['major_version'] + ' ' + data_frame[
        'network_type']
...
def flatten(long: pd.DataFrame):
    ...
    return long.pivot(
        index=['build_version', 'upstream_job', 'upstream_job_build',
               'start_date', 'end_date'],
