...
const SummaryBenchmarkExpandedRow = ({
  ...
  // Get the summary data for this benchmark (cached at benchmark level, not per config/iteration)
  const benchmarkDataKey = `${product}-${version}-${benchmark}`;
  const currentSummaryData = summaryData[benchmarkDataKey];
  ...
  const fetchSummaryData = useCallback(
    async (
      ...
      try {
        ...
        // Fetch data for the entire benchmark (all configs and iterations)
        await dispatch(
          fetchData(
            product,
            version,
            benchmark,
            null, // config not needed - API returns all configs
            null, // iteration not needed - API returns all iterations
            startDate,
            endDate,
          ),
        );
    ...
    [dispatch, hasFetchedData, currentSummaryData],
  );
  ...
  // Extract benchmark readiness from the loaded summary data
  const getBenchmarkReadiness = () => {
    if (!currentSummaryData) return null;
    const productData = currentSummaryData[product];
    if (!productData?.versions) return null;
    const versionData = productData.versions[version];
    if (!versionData?.benchmarks) return null;
    const benchmarkData = versionData.benchmarks[benchmark];
    return benchmarkData?.readiness || null;
  };

  const benchmarkReadiness = getBenchmarkReadiness();
