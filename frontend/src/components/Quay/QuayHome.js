...
export function QuayHome() {
    ...
    const [imagePushPull, setImagePushPull] = useState(searchParams.get('imagePushPull') || quayJobs.selectedImagePushPull)
    ...
    const sidebarComponents  = [
        ...
        {name: "Hit Size", onChange: setHitSize, value: hitSize, options:quayJobs.hitSize },
        {name: "Concurrency", onChange: setConcurrency, value: concurrency, options:quayJobs.concurrency },
        {name: "Image Push/Pulls", onChange: setImagePushPull, value: imagePushPull, options:quayJobs.imagePushPull }
    ]
    ...
    useEffect(() => {
        ...
        if(imagePushPull !== '') buildParams += `&imagePushPull=${imagePushPull}`
    ...
    }, [history, ciSystem, platform, benchmark, releaseStream, workerCount, hitSize, concurrency, imagePushPull, startDate, endDate])

    useEffect( ()=>{
        dispatch(updateQuayDataFilter({ciSystem, platform, benchmark, releaseStream, workerCount, hitSize, concurrency, imagePushPull}))
    }, [ ciSystem, platform, benchmark, releaseStream, workerCount, hitSize, concurrency, imagePushPull, dispatch ])
