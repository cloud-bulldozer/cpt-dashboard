...
const summaryReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case TYPES.SET_SUMMARY_DATA:
      // Store data at benchmark level (not per config/iteration) since API returns all data
      const dataKey = `${action.payload.product}-${action.payload.version}-${action.payload.benchmark}`;
      return {
        ...state,
        summaryData: {
          ...state.summaryData,
