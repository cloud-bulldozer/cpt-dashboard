...
const jobsSlice = createSlice({
    ...
    reducers: {
        ...
        updateQuayDataFilter: (state, action) => {
            const {ciSystem, platform, benchmark, releaseStream, workerCount, hitSize, concurrency, imagePushPull} = action.payload
            ...
            state.selectedImagePushPull = imagePushPull
            state.data = getQuayUpdatedData(original(state.copyData), platform, benchmark, releaseStream, workerCount, ciSystem, hitSize, concurrency, imagePushPull)
            Object.assign(state,  getQuaySummary(state.data))
        },
        updateQuayMetaData: (state, action) => {
            state.data = getQuayUpdatedData(action.payload.data, state.selectedPlatform, state.selectedBenchmark,
                ...
                state.selectedConcurrency, state.selectedImagePushPull)
            Object.assign(state,  getQuaySummary(state.data))
        },
